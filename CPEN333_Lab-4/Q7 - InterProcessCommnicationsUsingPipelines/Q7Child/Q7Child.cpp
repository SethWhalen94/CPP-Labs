...
int	main()
{
	...
	cout << "Child Process Opening Pipeline #1.....\n" ;
	// ============== NOTE:: Name of pipeline must be the same as it is in the parent!!!!!==============
	CPipe	p1("MyPipe1", 1024) ;		// Create a pipe 'p1' with the name "MyPipe"
	

	p1.Read(&i, sizeof(i)) ;				// Read the int 'i' from the pipe
	cout << "Child Process read integer = " << i << " from Pipeline #1.....\n";
	...
	cout << "Child Process read string = " << name << ", from Pipeline #1.....\n";	

	p1.Read(&mystruct, sizeof(mystruct)) ;	// Read the structure from the pipeline
	cout << "Child Process read [" << mystruct.x << "," << mystruct.y << "] from Pipeline #1.....\n";
