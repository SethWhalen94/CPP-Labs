...
int	main()
{
	...
	cout << "Child Process Opening Pipeline #2.....\n";
	// ============== NOTE:: Name of pipeline must be the same as it is in the parent!!!!!==============
	CPipe	p2("MyPipe2", 1024);		// Create a pipe 'p2' with the name "MyPipe1"


	p2.Read(&i, sizeof(i));				// Read the int 'i' from the pipe
	cout << "Child Process read integer = " << i << " from Pipeline #2.....\n";
	...
	cout << "Child Process read string = " << name << ", from Pipeline #2.....\n";

	p2.Read(&mystruct, sizeof(mystruct));	// Read the structure from the pipeline
	cout << "Child Process read [" << mystruct.x << "," << mystruct.y << "] from Pipeline #2.....\n";
